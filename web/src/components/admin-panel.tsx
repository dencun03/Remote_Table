...
const roleConfig: Record<string, { label: string; variant: 'default' | 'secondary' | 'destructive' | 'outline'; color: string }> = {
  ...
  user: { label: 'Пользователь', variant: 'secondary', color: 'text-muted-foreground bg-muted/10 border-border/20' },
}
  ...
  const handleRoleChange = async (userId: number, newRoleName: string) => {
    ...
    try {
      const res = await fetch('/api/admin/users', {
        ...
        body: JSON.stringify({ userId, roleId: role.id }),
      })
        ...
        {[
          ...
          <Card key={stat.label} className="border-border bg-card/50">
            <CardContent className="flex items-center gap-4 p-4">
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-muted ${stat.color}`}>
          ...
          <div className="max-h-[500px] overflow-y-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-card">
                <tr className="border-b border-border text-left text-xs font-medium uppercase tracking-wider text-muted-foreground/70">
                    ...
                    <tr key={user.id} className="group transition-colors hover:bg-muted/30">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="truncate font-medium text-foreground">
                              {user.username}
                              {isSelf && (
                                <span className="ml-2 text-[10px] text-muted-foreground/70">(вы)</span>
                              )}
                            </p>
                          ...
                          <div className="relative">
                            {changingRole === user.id ? (
                              ...
                              <select
                                ...
                                className="cursor-pointer appearance-none rounded-md border border-input bg-muted px-2 py-1 pr-6 text-xs font-medium text-foreground/80 hover:border-input focus:border-emerald-500 focus:outline-none"
                      ...
                      <td className="hidden py-3 pr-4 md:table-cell">
                        <div className="text-xs text-muted-foreground">
                          <span className="text-foreground/80">{user._count.createdTickets}</span> создано
                          <span className="mx-1 text-muted-foreground/40">/</span>
                          <span className="text-foreground/80">{user._count.assignedTickets}</span> назначено
                        </div>
                      </td>

                      {/* Created */}
                      <td className="hidden py-3 pr-4 text-xs text-muted-foreground/70 lg:table-cell">
                        {formatDate(user.createdAt)}
                      </td>

                      {/* Last login */}
                      <td className="hidden py-3 text-xs text-muted-foreground/70 lg:table-cell">
                      ...
                      <td className="py-3 pr-4 text-right">
                        <Badge
                          ...
                          className={`text-[10px] ${user.isActive ? 'border-emerald-500/30 text-emerald-400' : 'text-muted-foreground/70'}`}
                        >
                      ...
                      <td className="py-3 text-right">
                        {!isSelf && (
                          <Button
                            ...
                            className={`h-8 w-8 ${user.isActive ? 'text-muted-foreground/70 hover:text-red-400' : 'text-muted-foreground/70 hover:text-emerald-400'}`}
